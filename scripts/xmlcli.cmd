...
if /I "%RUN_NO_BUILD%"=="yes" set "USE_NO_BUILD=1"
if /I "%RUN_NO_BUILD%"=="on" set "USE_NO_BUILD=1"

if "%RUN_CONFIG%"=="" (
    if "%USE_NO_BUILD%"=="1" (
        dotnet run --project "%REPO_ROOT%\src\%CLI_PROJECT%" --no-build -- %*
    ) else (
        dotnet run --project "%REPO_ROOT%\src\%CLI_PROJECT%" -- %*
    )
) else (
    if "%USE_NO_BUILD%"=="1" (
        dotnet run --project "%REPO_ROOT%\src\%CLI_PROJECT%" -c "%RUN_CONFIG%" --no-build -- %*
    ) else (
        dotnet run --project "%REPO_ROOT%\src\%CLI_PROJECT%" -c "%RUN_CONFIG%" -- %*
    )
)
set "EXIT_CODE=%ERRORLEVEL%"
