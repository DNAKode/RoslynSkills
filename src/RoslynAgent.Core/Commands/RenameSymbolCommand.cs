...
public sealed class RenameSymbolCommand : IAgentCommand
{
    ...
    public async Task<CommandExecutionResult> ExecuteAsync(JsonElement input, CancellationToken cancellationToken)
    {
        ...
        ISymbol? targetSymbol = GetSymbolForToken(anchorToken, semanticModel, cancellationToken);
    ...
    {
        ISymbol? symbol = GetSymbolForToken(token, semanticModel, cancellationToken);
        if (symbol is null)
        {
            return false;
        }
    ...
    private static ISymbol? GetSymbolForToken(
        SyntaxToken token,
        SemanticModel semanticModel,
        CancellationToken cancellationToken)
    {
