...
""",
            ...
            case "ctx.member_source":
                    ...
                    !int.TryParse(positionalArgs[2], out int column))
                {
                    error = ErrorEnvelope(
                        commandId: "cli",
                        code: "invalid_args",
                        message: BuildUsageMessage(commandId, "ctx.member_source <file-path> <line> <column> [member|body] [--option value ...] OR ctx.member_source <file-path> --member-name <name> [--option value ...]"));
                    return false;
                }
    ...
    private static string BuildClaimStatusSuffix(JsonElement element, bool wroteFile)
    {
        ...
        if (!TryGetBool(claimStatus, "claimed", out bool claimed) || !claimed)
        {
            return ", unclaimed";
        }

        if (claimStatus.TryGetProperty("matching_claims", out JsonElement matchingClaims) &&
            matchingClaims.ValueKind == JsonValueKind.Array &&
            matchingClaims.GetArrayLength() > 0)
        {
            JsonElement firstClaim = matchingClaims[0];
            if (TryGetString(firstClaim, "claim_id", out string claimId) &&
                !string.IsNullOrWhiteSpace(claimId))
            {
                string shortClaimId = claimId.Length > 8 ? claimId[..8] : claimId;
                return $", claimed={shortClaimId}";
            }
        }
        ...
        bool requireSolution)
    {
        ...
        await WriteEnvelopeAsync(stdout, new CommandEnvelope(
            ...
            Data: new
            {
                ...
                steps,
                next = ok
                    ? "Continue one narrow, testable C# slice: one behavior, one primary claimed file when feasible, one focused test first. If focused tests already pass on existing dirty work and no new mutation is needed, report validation-only and choose a different smallest unimplemented increment unless the user asked only to validate. If the slice grows past two C# files or more than one behavior, stop after the smallest validated increment and report the remaining work. Run roscli semantic reads sequentially; do not launch parallel ctx.*, nav.*, edit.*, build, or test commands against the same workspace. Use roscli for .cs context, claim before mutation, validate with focused tests, and release claims."
    ...
    private static string BuildCSharpStartGuide(bool supervised, string? solutionPath, string bootstrapCommand = "roscli csharp-start")
    {
        ...
        StringBuilder sb = new();
        sb.AppendLine($"# {bootstrapCommand}");
        sb.AppendLine();
        ...
        if (supervised)
        {
            ...
            sb.AppendLine("```text");
            sb.AppendLine($"Run exactly this one command before any docs or C# exploration, then stop and report its Summary plus the three step summaries: `roscli agent-begin --solution {preloadTarget}`.");
            sb.AppendLine("```");
            sb.AppendLine("Turn 2B prompt after the agent-begin report:");
            sb.AppendLine("```text");
            sb.AppendLine("Continue one narrow, testable C# slice using the `next` instruction from agent-begin. First-slice budget: one behavior, one primary claimed file when feasible, one focused test before wider validation; if focused tests already pass on existing dirty work and no new mutation is needed, report validation-only and choose a different smallest unimplemented increment unless the user asked only to validate; if work expands past two C# files or more than one behavior, stop at the smallest validated increment and report follow-up work. Use roscli for .cs context, edits, and post-edit anchors: compact ctx.file_outline filters, ctx.member_source with small focus windows (start 3-12 lines, not 80+), describe-command before the first Roslyn edit command, then edit.claim claim for every file before mutation and run the edit command if mutation is needed. If ctx.member_source returns focus=not-found, stop and rerun with a better literal, smaller context window, or include_source_text=false before reading larger source. If ctx.search_text finds a method reference and you need callers/routing, use nav.find_invocations or nav.call_hierarchy on that line/column instead of guessing router members through broad outlines. Treat edit summaries that include `claimed=<id>` as mutation-time claim evidence; `unclaimed` means stop and claim before further C# mutation. If subagents are used, assign disjoint claimed files; serialize shared-file/member edits through one owner and guarded expected_text. Run roscli and dotnet commands sequentially per repo; do not launch parallel semantic reads, edits, builds, or tests against the same workspace. If a broad ctx.search_text returns many matches, stop broad searching and narrow with member_name_contains or member_source focus_text. When consulting non-C# upstream/reference files, first locate one likely file with `rg -l -m 20 <literal> <reference-root>`; then inspect only that file with `rg -n -C 2 -m 20 <literal> <file>`. Do not run alternation-heavy rg across whole .external/reference trees. For small multiline `edit.replace_in_member` old_text/new_text, use `roscli run edit.replace_in_member --input-stdin` JSON instead of direct shell option values; for large block/member/body rewrites, use `ctx.member_source --include-edit-target-text true` and `edit.batch_exact` replace_span with expected_text instead of replaying huge old_text/new_text in the transcript. Use ctx.search_text or ctx.member_source for .cs closeout line anchors; for member closeout anchors prefer `ctx.member_source --focus-text <literal> --context-lines-before 1 --context-lines-after 1 --include-source-text false` so the transcript carries line/focus metadata without replaying source. Do not use rg/git diff/Get-Content on .cs files. Report startup evidence explicitly from agent-begin step summaries. Report any .cs fallback explicitly.");
            sb.AppendLine("```");
            sb.AppendLine("If the agent starts docs or C# work before the agent-begin transcript appears, interrupt and rerun Turn 1; do not treat prose promises as compliance.");
        }
        ...
        sb.AppendLine("- If `ctx.search_text` locates a method call/reference and you need the enclosing router or callers, run `nav.find_invocations <file> <line> <column> --brief true --max-results 20` or `nav.call_hierarchy <file> <line> <column> --direction incoming --max-depth 1 --brief true`; do this before raising outline/member-source windows.");
        ...
        sb.AppendLine("- When comparing against non-C# upstream/reference sources, do a two-step bounded lookup: first `rg -l -m 20 <literal> <reference-root>` to pick one file, then `rg -n -C 2 -m 20 <literal> <file>` against that file only.");
        sb.AppendLine("- Avoid alternation-heavy `rg` across whole `.external`/reference trees; if output is more than one screen, stop and narrow to one file before reading more.");
        ...
        sb.AppendLine("roscli edit.replace_in_member tests/MyTests.cs --member-name TargetTest --old-text \"Assert.Equal(1, value);\" --new-text \"Assert.Equal(2, value);\" --preview-chars 256  # one-line snippets only");
        ...
        sb.AppendLine("```");
        sb.AppendLine();
        sb.AppendLine("Use `edit.replace_in_member` for small exact changes inside one member. Direct old-text/new-text options are for one-line snippets only; for small multiline replacements, send JSON with `roscli run edit.replace_in_member --input-stdin` or `--input @payload.json`. Successful write summaries include `claimed=<id>` when an active claim covered the file, or `unclaimed` when no active claim covered the mutation. Use `edit.batch_exact` with `replace_span` and `expected_text` for coordinated whole-member, large block, same-member multi-edit, or multi-file edits; get spans from `ctx.member_source --include-edit-target-text true` instead of copying huge old_text/new_text blocks. Do not run parallel edit commands against the same member from an older `ctx.member_source` read. Use `edit.insert_text` for exact-anchor insertions, but first run `describe-command edit.insert_text` and avoid copied multiline block anchors; prefer a short unique anchor line from `ctx.member_source` or `ctx.search_text`.");
        sb.AppendLine();
