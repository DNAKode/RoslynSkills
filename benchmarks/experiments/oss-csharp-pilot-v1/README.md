...
1. Validate manifest:
   - `dotnet run --project src/RoslynSkills.Benchmark -- agent-eval-validate-manifest --manifest benchmarks/experiments/oss-csharp-pilot-v1/manifest.json`
2. Generate worklist and pending templates:
   - `dotnet run --project src/RoslynSkills.Benchmark -- agent-eval-init-runs --manifest benchmarks/experiments/oss-csharp-pilot-v1/manifest.json --runs <runs-dir>`
3. Execute control/treatment runs and save JSON outputs under `<runs-dir>`.
4. Validate runs:
   - `dotnet run --project src/RoslynSkills.Benchmark -- agent-eval-validate-runs --manifest benchmarks/experiments/oss-csharp-pilot-v1/manifest.json --runs <runs-dir>`
5. Score:
   - `dotnet run --project src/RoslynSkills.Benchmark -- agent-eval-score --manifest benchmarks/experiments/oss-csharp-pilot-v1/manifest.json --runs <runs-dir>`
