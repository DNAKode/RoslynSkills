...
Note (LSP MCP adapter):

- Earlier LSP cells used external `cclsp`; current workspace includes an internal bridge `scripts/csharp-lsp-mcp.cmd` backed by `csharp-ls`.
- Early runs show LSP reference queries can be slow/timeout-heavy; treat LSP lanes as experimental until warm-up + timeouts are stabilized.
...
| --- | --- | --- | ---: | ---: | ---: | ---: | --- |
| roscli-vs-base-task001-spark-low-brief | gpt-5.3-codex-spark | control-text-only | n/a | 830,221 | 45 | 0 | Yes (paired) |
| roscli-vs-base-task001-spark-low-brief | gpt-5.3-codex-spark | treatment-roslyn-published-cache | n/a | 640,223 | 24 | 1+ | Yes (paired) |
...
- A clean paired control vs roscli large-task replicate now exists (roscli-vs-base-task001-spark-low-brief), but more replicates are still required before strong claims.
