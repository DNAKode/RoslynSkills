...
- workspace-mode extraction now supports JSON envelope parsing plus escaped/unescaped text fallback for concatenated `roscli run ...` outputs, compatible with Windows PowerShell `ConvertFrom-Json` behavior (no `-Depth` dependency).
- interpret `workspace/ad_hoc = 0/0` as "no workspace-context-bearing payloads were observed" (not automatically as ad-hoc fallback).
- for `-TaskShape single-file`, `workspace_context.mode=ad_hoc` is expected unless a workspace is explicitly provided; use `-TaskShape project` for workspace-binding claims.
