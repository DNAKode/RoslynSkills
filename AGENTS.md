...
- `2026-05-16`: Live FrankenTui.NET hot-server trial exposed adoption failures before command failures -> Global `roscli` packaging, daemon command discoverability, and `workspace.status` shorthand had to work before agents would stay on the Roslyn path -> Treat global-tool smoke plus `list-commands`/`describe-command` daemon coverage as mandatory before supervised external-repo trials.
- `2026-05-16`: `ctx.member_source` became agent-usable only after default hot-workspace aliasing and line/column help were explicit -> `workspace.preload` now persists alias `default` and command guidance tells agents to verify `workspace_cache_mode=process_hot` -> Optimize context commands for no-handle steady-state usage after one solution preload.
- `2026-05-16`: Multi-agent edit coordination needs a Roslyn-native reservation layer separate from mutation -> Added `edit.claim` with repo-local claim state for file/member ownership while leaving semantic edits to transaction/session commands -> Require claim-before-edit guidance in supervised multi-agent runs and measure remaining text-patch mutation as a command-surface gap.
- `2026-05-16`: Exact replacement is not the same as ergonomic insertion in live agent behavior -> Added `edit.replace_text` as a structured mutation bridge, but the FrankenTui.NET `.23` trial still used the patch editor for line insertions after a known anchor -> Add an insert-oriented command/recipe (`insert after/before exact anchor`) and keep mutation-channel telemetry separate from navigation/claim adoption.
- `2026-05-16`: Anchor-based insertion is the next pit-of-success mutation primitive -> Added `edit.insert_text` with exact-anchor before/after insertion, ambiguity failure, direct shorthand, diagnostics, and guidance -> In live trials, steer small evidence/assertion additions to `edit.insert_text` before allowing patch fallback.
- `2026-05-16`: Live `.24` FrankenTui.NET trial showed agents can use roscli for hot navigation/claims/diagnostics but still mutate `.cs` via patch editor -> Added `ctx.member_source.edit_workflow` guidance with claim-first and preferred mutation commands in the context payload itself -> Put edit-command choice at the moment the agent is reading source, not only in separate docs/prompts.
- `2026-05-16`: Claim cleanup must be reliable for supervised multi-agent editing -> `edit.claim release <claim_id>` previously released zero claims unless owner/path matched -> Treat claim IDs as explicit release tokens in direct CLI shorthand while keeping path release owner-scoped by default.
- `2026-05-16`: `.25` supervised run produced the first clear Roslyn mutation adoption: the agent read `ctx.member_source.edit_workflow`, claimed files, and used `edit.replace_text` for C# changes -> Context-local edit guidance is an effective adoption lever -> Next improve edit-command diagnostics so mutation responses can use hot workspace/project context instead of parse-only file diagnostics.
- `2026-05-16`: Exact text edits need project-aware feedback to stay in the Roslyn path -> `edit.replace_text`/`edit.insert_text` now reuse workspace-backed updated-source diagnostics and carry workspace context in the edit response -> Prefer one command round-trip for mutate+diagnose when a hot/default workspace is available.
- `2026-05-16`: `.26` live trial exposed stale hot context after exact edit writes -> `ctx.member_source` could serve pre-edit source while disk search saw the new file -> Exact write commands now refresh matching hot workspaces and report `hot_workspace_refresh`; use that response before re-reading context.
- `2026-05-16`: Supervised agents hid failures by batching several exact edit commands in one shell block -> Successful later commands obscured earlier `old_text_not_found`/`old_text_ambiguous` failures -> Prefer one edit command per observation step until a multi-operation exact-edit transaction reports per-operation status atomically.
- `2026-05-16`: `.27` supervised FrankenTui.NET run validated hot exact-edit refresh but repeated the hidden-partial-failure pattern and devolved into dummy `old_text` probes -> Added `edit.batch_exact` with atomic-by-default replace/insert operations and per-operation results -> Prefer `edit.batch_exact` over shell-chaining multiple exact edit commands.
- `2026-05-16`: `.28` supervised FrankenTui.NET batch run contained partial-write risk but whole-member `old_text` copied from `ctx.member_source` still failed due fragile multiline text transfer -> Added `ctx.member_source.edit_target` spans and `edit.batch_exact` `replace_span` operations with optional `expected_text` guards -> Prefer span-based batch edits for large member replacements and reserve exact text for small unique snippets.
- `2026-05-16`: `.29` supervised FrankenTui.NET span run validated atomic `replace_span` plus hot refresh, but repeated whole-member edits could double indentation when agents copied line-oriented `source.text` into span `new_text` -> Added `edit_target.exact_span_text` and trivia/preserved-prefix rules to `ctx.member_source` guidance -> Use exact span text, not full line snippets, as the replacement base for span edits.
- `2026-05-16`: `.30` payload smoke showed the implementation guidance was stronger than the bootstrap examples -> Added quickstart/llmstxt span-member edit recipes using `include_edit_target_text=true`, `edit.claim`, and `edit.batch_exact replace_span` -> Keep top-level onboarding synchronized with live-trial command lessons, not only describe-command details.
- `2026-05-16`: Cross-repo supervision from the RoslynSkills host pane exposed daemon lifecycle drift -> `workspace.preload` keyed the hot host to the caller cwd instead of the target FrankenTui.NET solution root -> Infer daemon repo root from the workspace path for `workspace.use/preload` unless `--repo-root` is explicit.
- `2026-05-16`: Cross-repo smoke then exposed file-command routing drift -> `ctx.member_source` launched from the host pane still searched the caller cwd daemon after preloading a sibling repo -> Infer daemon repo root from `file_path`/`workspace_path`, including nested batch edit operations, for daemon-capable tool calls.
- `2026-05-16`: `.33` supervised indentation repair succeeded but required agent inference about trivia spans -> Default `ctx.member_source` spans preserve line prefixes outside `span_start`, so prefix repairs need `include_trivia=true` -> Surface an explicit `prefix_edit_rule` in `edit_target.trivia` whenever the preserved prefix may be the edit target.
- `2026-05-16`: `.34` supervised product edits still omitted `expected_text` from copied span operations -> Multi-agent span edits need stale-coordinate guards by default -> Include `expected_text` in `ctx.member_source.edit_target.replace_span_operation` whenever exact span text is available and untruncated.
- `2026-05-16`: Supervised claim cleanup showed agents naturally try resource-list verbs -> `edit.claim list` failed even though it meant `status` -> Accept `list` as a status alias and advertise it in help.
- `2026-05-16`: Large test-file navigation still created outline noise -> `ctx.file_outline` on `ShowcaseShellTests.cs` was useful but too broad for focused parity work -> Add `member_name_contains` and `type_name_contains` filters so agents can get compact line/column anchors before `ctx.member_source`.
- `2026-05-16`: Huge member bodies still created Roslyn-to-text fallback pressure -> Add `ctx.member_source focus_text` to return a small line window around a literal match while retaining member-level edit target metadata -> Prefer focused Roslyn windows over `rg`/file reads for branch-level work inside long C# members.
- `2026-05-16`: Focused member reads can accidentally request truncated whole-member edit text -> Mark truncated `exact_span_text` as unsafe for whole-target `replace_span` and steer focused edits to small exact mutations from `source.text` -> Keep focus windows as exploration tools unless the agent explicitly requests an untruncated replacement base.
- `2026-05-16`: `.39` live use showed focused reads still inherited whole-target text by default -> Make `ctx.member_source focus_text` default `include_edit_target_text=false` unless explicitly requested -> Keep huge-member exploration compact and reserve replacement bases for deliberate edit construction.
- `2026-05-16`: Ambiguous exact text failures were safe but underspecified -> Add `edit.batch_exact` per-operation `recovery_hint` for ambiguous/missing exact text and anchors -> Teach agents to switch to `ctx.member_source` span anchoring instead of retrying copied text blindly.
- `2026-05-16`: Stale line anchors can silently land on adjacent members during focused reads -> Add `ctx.member_source` preview focus state (`focus=matched:<line>` / `focus=not-found:<text>`) -> Make wrong-member or failed-focus reads visible before agents reason from full payloads.
- `2026-05-16`: Large test-file line anchors drift after every supervised edit -> Add `ctx.member_source member_name` for unique member-name anchoring -> Let agents move directly from filtered outlines to source without copying stale line/column coordinates.
- `2026-05-16`: `.43` member-name anchoring worked, but small recovery edits still required shell glue to convert member reads into span replacements -> Add `edit.replace_in_member` for claim-first exact replacements scoped to a unique member/body with line-ending normalization -> Prefer it for small assertion/property edits inside known members; keep guarded `replace_span` for whole-member and multi-file edits.
- `2026-05-16`: `.44` scoped member replacement worked first try, but the preview did not show where the exact match landed -> Add `edit.replace_in_member.matches[]` with line/column/offset/length and include the first line in CLI previews -> Make scoped exact edits faster to audit during supervised multi-agent runs.
- `2026-05-16`: `.45` match-line previews helped audit scoped edits, but line/column alone did not show what text matched -> Add bounded `matches[].text_preview` for `edit.replace_in_member` -> Keep mutation logs self-contained enough for quick supervision without rereading source.
- `2026-05-16`: `.46` text previews showed the matched snippet but not the replacement -> Add bounded `matches[].new_text_preview` for `edit.replace_in_member` -> Make scoped exact edit logs show compact before/after intent.
- `2026-05-16`: `.47` before/after previews helped scoped edit audit, but long assertion replacements could truncate before the inserted fields -> Add `edit.replace_in_member.preview_chars` for bounded caller-controlled previews -> Let agents raise audit detail on long snippets without rereading source or increasing default payloads.
- `2026-05-16`: `.48` caller-controlled previews helped, but long shared prefixes could still consume the audit window before inserted assertion fields -> Bias truncated `edit.replace_in_member` previews toward the first changed character -> Make compact previews show the semantic delta by default.
- `2026-05-16`: `.49` change-biased previews helped, but agents still had to visually compare strings to locate the divergence -> Add `first_changed_offset` and line/column deltas to `edit.replace_in_member.matches[]` -> Give tooling a structured jump point for scoped edit audit.
- `2026-05-16`: `.50` first-change offsets made scoped edits machine-locatable, but logs still needed compact delta-only text -> Add `old_change_preview`/`new_change_preview` to `edit.replace_in_member.matches[]` -> Let agents report the actual changed region without parsing offsets or rereading source.
- `2026-05-16`: `.51` delta-only previews helped reporting, but downstream audit tooling still had to compute absolute jump coordinates -> Add old/new change line/column fields to `edit.replace_in_member.matches[]` -> Make scoped edit deltas directly navigable.
- `2026-05-16`: First `/new` acquisition trial with sparse prompt failed to trigger roscli onboarding; the fresh agent used `git diff`, `Get-Content`, and direct edits on `.cs` before any visible roscli call -> Promote a concrete fresh-C# first-minute path in `quickstart`/`PIT_OF_SUCCESS.md` with workspace preload, `ctx.file_outline`, `ctx.member_source`, `edit.replace_in_member`, and explicit `.cs` fallback guardrails -> Make minimal prompts point to executable onboarding that competes with default text-first habits.
- `2026-05-16`: Second `/new` acquisition trial still ignored JSON quickstart/PIT pointers and opened with `git diff`/`rg` over `.cs` files -> Added `roscli csharp-start` as a compact markdown entry point focused on `ctx.member_source`, claim-first edits, and multi-agent stale-context guards -> Test whether a single short command beats broad docs for fresh-agent tool acquisition.
- `2026-05-16`: Later `/new` trials showed `csharp-start` worked only after the operator forced that single command; mixed "run this first and continue work" prompts still let agents begin with `git diff`/`rg` -> Added `roscli csharp-start --supervised` with a two-turn transcript-verification protocol -> Treat first-command compliance as an observed gate, not a prose promise.
- `2026-05-16`: The supervised protocol produced clean `ctx.member_source` adoption with no `.cs` fallback, but the agent skipped explicit hot-solution preload before context reads -> Updated supervised Turn 2 to require `workspace.preload <solution> --alias default --require-solution true` before `ctx.*` -> Keep hot-server activation in the acquisition path, not only in later examples.
- `2026-05-16`: The next supervised run used claims, Roslyn context, and Roslyn mutation successfully, but first tried an edit command with the wrong option shape before correcting via `describe-command edit.replace_in_member` -> Updated supervised Turn 2 to require `describe-command` before the first Roslyn edit command -> Move schema discovery ahead of the first mutation attempt.
- `2026-05-16`: A later supervised run kept mutation Roslyn-native but used `rg` on a `.cs` file only to find the changed assertion line for closeout -> Updated supervised Turn 2 and fallback guidance to require `ctx.search_text`/`ctx.member_source` for post-edit `.cs` anchors -> Treat reporting/audit reads as part of the Roslyn-first workflow, not an exempt shell step.
- `2026-05-16`: Supervised FrankenTui rounds still required the operator to manually replace the solution placeholder in Turn 2 -> Added `roscli csharp-start --supervised --solution <path>` to emit concrete `workspace.preload` examples -> Reduce human prompt shaping while preserving the two-turn acquisition protocol.
- `2026-05-16`: Supervised FrankenTui Quake-keyboard slice showed clean roscli edit adoption but burned context on repeated broad `ctx.search_text` and large outlines before converging on `ctx.member_source` -> Updated `csharp-start`, command hints, and pit-of-success docs to cap broad searches, prefer filtered outlines/focused member windows, and assign stable claim owners for subagents -> Treat context-volume churn as a tooling defect even when agents avoid text fallback.
- `2026-05-16`: `.61` supervised FrankenTui guided-tour slice validated the capped-search guidance: the agent stopped after a capped broad search, shifted to focused outlines/member-source, used claim-first `edit.replace_in_member`, released claims, and kept closeout anchors in roscli -> Next reduce outline noise when `member_name_contains` is too generic by warning or suggesting a narrower filter before returning dozens of members.
- `2026-05-16`: Minimal-prompt `/new` FrankenTui acquisition showed roscli edit adoption without claim adoption: the agent used `edit.replace_text`/`edit.replace_in_member` but skipped `edit.claim claim` after only listing claims -> Add `claim_status` to exact mutation responses and mark CLI summaries `unclaimed` when writes happen without an active file claim -> Put ownership feedback in the mutation result, where agents cannot miss it as easily.
- `2026-05-16`: Fresh FrankenTui run repeatedly tried intuitive `ctx.search_text --file-path ... --text ...` calls and hit `invalid_args` before falling back to other context paths -> Added direct CLI aliases `--text`/`--query` and `--file-path`/`--path` for `ctx.search_text` -> Treat observed agent command guesses as interface evidence and absorb low-risk aliases when they preserve the schema contract.
- `2026-05-16`: `.63` FrankenTui supervised round validated the new `ctx.search_text --file-path ... --text ...` alias path and produced a claim-first test-only edit with no `.cs` fallback, but the driver prompt still sent the agent looking for `docs/PIT_OF_SUCCESS.md` inside the target repo -> Future cross-repo prompts should point only to `roscli csharp-start`/`describe-command` unless the target repo explicitly contains the referenced docs -> Keep onboarding guidance portable across sibling panes.
- `2026-05-17`: Next fresh FrankenTui run acquired roscli from a portable minimal prompt and used claim-first edits successfully, but spent a large payload on broad `ctx.search_text "evidence" --max-results 80 --context-lines 0/1` before narrowing -> Add `ctx.search_text.result_guidance` for broad/truncated searches with concrete narrowing commands -> Put payload-control advice in the command result that creates the cost.
- `2026-05-17`: The following supervised round stayed on roscli for `.cs` content but used `git diff --stat` to orient around an already-dirty FrankenTui worktree -> Add `ctx.changed_files` for changed-file grouping without reading contents and make it part of `csharp-start`/pit guidance -> Treat dirty-worktree orientation as a roscli command surface, especially when multiple agents may have outstanding file claims and edits.
- `2026-05-17`: The `.65` FrankenTui run validated `ctx.changed_files` adoption but exposed stale-context risk from multiple same-member edit commands in one long test method -> Surface same-member multi-edit guidance in `ctx.member_source`, `describe-command edit.replace_in_member`, `csharp-start`, and pit docs -> Require agents/subagents to combine same-member changes into one guarded edit after a fresh member read.
- `2026-05-17`: The `.66` FrankenTui run used `ctx.changed_files` but then spent large payloads on broad outlines before hot solution preload -> Make `ctx.changed_files.suggested_next_steps` start with solution preload when discoverable and recommend filtered outlines/member-source windows instead of high `--max-members` scans -> Put first follow-up behavior in the dirty-worktree command result itself.
- `2026-05-17`: The `.67` FrankenTui run followed preload and filtered outlines, but `ctx.member_source` returned a 1447-line member when `focus_text` missed -> Cap large missing-focus member payloads and emit explicit `payload_guidance` search/outline next steps -> Treat focus misses in large members as a command-surface steering event, not a normal full-member read.
- `2026-05-17`: First new-cycle sparse-prompt FrankenTui trial ignored `csharp-start` and opened with `.cs` `git diff` plus status/test loops -> Move the C# first-minute guard to the top of `roscli --help` so even generic help discovery says to run `csharp-start` before `.cs` text reads and then `ctx.changed_files` in dirty repos -> Treat bare help as a safety rail, not just a command catalog.
- `2026-05-17`: Second sparse-prompt FrankenTui trial still skipped `roscli --help`/`csharp-start` and went straight to `.cs` `git diff`, `rg`, and `Get-Content` -> Add `roscli agent-start` as a purpose-built supervised first-command bootstrap that emits the existing two-turn protocol by default and surface it in help/quickstart -> Treat fresh-pane acquisition as an operator protocol requiring transcript evidence, not as passive documentation.
- `2026-05-17`: `agent-start` bootstrap successfully shifted the FrankenTui pane into roscli-first navigation/editing, but the second-turn prompt still left room to skip `ctx.changed_files` and concrete preload discovery -> Auto-detect a single top-level solution for `agent-start` when `--solution` is omitted and put `ctx.changed_files` plus claim-before-mutation directly in the supervised turn-2 text -> Make the compliant path copy/paste complete after the first command.
- `2026-05-17`: The follow-up `agent-start` trial stayed roscli-first and released claims, but final reporting did not prove whether the startup commands ran -> Add required closeout evidence for `edit.claim list`, `ctx.changed_files`, and `workspace.preload` to the generated Turn 2 prompt -> Measure startup compliance explicitly instead of inferring it from later command mix.
- `2026-05-17`: Repeated fresh-pane trials proved `agent-start` works, but its embedded Turn 1 text still named `roscli csharp-start`, creating avoidable command mismatch in the acquisition transcript -> Make the supervised guide command-aware so `agent-start` tells agents to verify `roscli agent-start` while `csharp-start --supervised` keeps its own command -> Keep bootstrap prompts internally consistent.
- `2026-05-17`: Supervised `agent-start` title mismatch and member-source cold-start friction observed in FrankenTui live pane -> Make `agent-start` print its own H1, document `ctx.search_text` -> `ctx.file_outline` -> `ctx.member_source` when only a type/member name is known, and require disjoint edit claims for subagents with shared-file edits serialized through one owner -> Keep first-command evidence unambiguous and make multi-agent C# edits fail closed instead of racing on stale member context.
- `2026-05-17`: Long-running watcher/roscli processes and a parallel source-backed command attempt created host process pressure and briefly locked the old global tool package during release -> Add sequential-per-repo guidance to supervised startup text -> Keep fresh-agent acquisition fast by making the safe execution model explicit: no parallel semantic reads, edits, builds, or tests against the same workspace.
- `2026-05-17`: `.75` fresh FrankenTui run acquired roscli cleanly and used claim-first C# edits, but repeated large `ctx.member_source` windows around one member before editing -> Tighten supervised startup guidance to start with small 3-12 line focus windows and avoid 80+ line reads unless deliberately replacing a whole target -> Treat context-window size as part of acquisition quality, not just command adoption.
- `2026-05-17`: `.76` fresh FrankenTui run recovered from a C# `ctx.member_source member_not_found` only after extra searching, while self-hosted attempts to edit that command also exposed roscli hangs -> Add member-not-found recovery guidance directly to the command error and log the hang as a reliability defect -> Put the next best Roslyn command in the failure envelope instead of relying on agent inference.
- `2026-05-17`: `.77` fresh FrankenTui run completed with roscli edits, but first tried a multi-line `edit.replace_in_member` through direct shorthand with PowerShell here-string syntax and hit invalid_input -> Add explicit `describe-command edit.replace_in_member` guidance to use `--input @payload.json` or `--input-stdin` for multi-line snippets -> Keep direct shorthand scoped to short one-line edits.
- `2026-05-17`: `.78` fresh FrankenTui run validated multiline JSON/stdin guidance, but direct one-line `edit.replace_in_member` still failed on comma-bearing C# method-call snippets because shorthand parsing treated comma text as arrays -> Restrict comma-list parsing to compact no-whitespace values and regression-test method-call replacements -> Keep direct one-line edit shorthand usable while routing multiline edits through JSON/stdin payloads.
- `2026-05-17`: `.79` fresh FrankenTui run kept C# work on roscli and validated claim/closeout discipline, but unbounded upstream Rust `rg -C 3` dumped a 1447-line reference payload before the agent returned to C# edits -> Add bounded non-C# upstream/reference search guidance to supervised startup -> Control transcript cost around port-reference lookup without expanding roscli beyond C# semantics.
- `2026-05-17`: `.80` fresh FrankenTui run completed cleanly and validated direct one-line edit shorthand, but `ctx.search_text --pattern ... --file-glob "*.cs"` failed once because agents expect file_glob to imply current-workspace scope -> Infer the single top-level solution for direct `ctx.search_text` when no explicit scope is provided -> Remove a predictable retry while keeping schema-first scoping for ambiguous repos.
- `2026-05-17`: `.81` fresh FrankenTui run validated inferred text-search scope, but after a zero-result filtered outline the agent escalated to `ctx.file_outline --max-members 80`, producing a 714-line payload -> Add `ctx.file_outline.result_guidance` for zero-match filters and large outlines -> Put payload-control recovery in the outline result itself.
- `2026-05-17`: `.82` fresh FrankenTui run followed zero-match outline guidance and avoided broad outline escalation, but smoke output showed suggested follow-up commands used only the basename of nested files -> Preserve the supplied file path in `ctx.file_outline.result_guidance` suggested commands -> Make recovery commands executable from repo root.
- `2026-05-17`: `.83` fresh FrankenTui run used `ctx.search_text --file-glob ShowcaseInteractiveProgram.cs` intending a single-file search, but direct shorthand ignored the alias and searched default `**/*.cs` scopes -> Map `--file-glob`/`--glob` to `include_globs` and document bare filename matching -> Keep printed startup examples executable exactly as written so narrowing options do not silently fail into broad C# scans.
- `2026-05-17`: `.84` smoke usage showed direct `ctx.search_text --pattern "Call(a, b)"` still split comma-bearing C# literals into pattern arrays -> Make comma-list parsing option-aware and preserve source-like text options as strings -> Treat every generic CLI coercion rule as suspect until tested against C# snippets.
- `2026-05-17`: `.85` fresh FrankenTui round stayed roscli-first but repeated broad `ctx.search_text` and `ctx.file_outline` calls consumed large payloads before a focused edit -> Cap brief broad-search match payloads while preserving `total_matches`/omitted counts and steer agents to narrow before raising caps -> Optimize not just command adoption but the transcript trajectory after adoption.
- `2026-05-17`: Split Turn 2A/2B startup made `agent-begin` evidence auditable, but mutation-time claim acquisition could still scroll out before review -> Make write summaries emit positive `claimed=<id>` as well as `unclaimed` -> Treat edit result summaries as the durable audit point for claim-before-mutation compliance.
- `2026-05-17`: `.96` fresh FrankenTui run validated positive claim summaries for replace edits but exposed missing claim summaries on `edit.insert_text` -> Add `claim_status` and CLI claim suffix to insert writes -> Keep audit semantics consistent across exact edit primitives.
- `2026-05-17`: `.97` fresh FrankenTui run stayed roscli-first but churned through focus misses and broad outlines while trying to find router/caller paths from method references -> Surface `nav.find_invocations`/`nav.call_hierarchy` in startup guidance after `ctx.search_text` method hits -> Prefer semantic caller/routing navigation over guessing enclosing members with large outline windows.
- `2026-05-17`: `.98` fresh FrankenTui run used claim-first JSON edits correctly but replayed a huge multiline `old_text`/`new_text` block through `edit.replace_in_member` -> Tighten startup guidance to reserve replace_in_member multiline JSON for small snippets and use `edit.batch_exact` replace_span for large block/member/body rewrites -> Optimize transcript size and stale-context safety after agents acquire roscli.
- `2026-05-17`: `.99` fresh FrankenTui run stayed roscli-first and completed cleanly, but guidance-only span-edit steering is easy to miss at mutation time -> Add `edit.replace_in_member.result_guidance` when old/new payloads are large, pointing to `ctx.member_source --include-edit-target-text true` plus `edit.batch_exact replace_span` -> Put token/stale-context correction in the result envelope that observes the costly pattern.
- `2026-05-17`: `.100` fresh FrankenTui run stayed roscli-only and validated claimed edit summaries, but an exact `ctx.member_source --member-name` miss in a large test file forced extra recovery inference -> Return structured `recovery_hint.suggested_commands` with file-specific `ctx.file_outline`/`ctx.search_text` on `member_not_found` -> Make failure recovery copy-ready instead of prose-only.
- `2026-05-17`: `.101` fresh FrankenTui run completed a focused test-only slice quickly, but `edit.replace_text` introduced a same-line `[Fact] public` formatting join that required a second repair edit -> Add `edit.replace_text.result_guidance` for suspicious C# attribute/member newline joins -> Surface formatting hazards immediately after exact text edits.
- `2026-05-17`: `.102` fresh FrankenTui run completed a Kanban control-key slice with claimed roscli edits, but a 12+ line `edit.replace_in_member` JSON payload still avoided the large-rewrite guidance threshold -> Lower the scoped-replace line threshold from 20 to 10 -> Catch transcript-expensive member edits earlier and steer to span edits sooner.
