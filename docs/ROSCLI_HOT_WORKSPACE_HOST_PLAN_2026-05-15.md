...
The `.45` supervised round confirmed the line preview was useful: the agent cited `line=3586` and `line=3588` while validating a perf HUD assertion addition and correction. The next audit gap was that line/column alone still required opening the full JSON or rereading the member to see what matched. `matches[]` now includes a bounded single-line `text_preview` for the matched old text so scoped edit logs can be reviewed without another source read.

The `.46` supervised round confirmed `text_preview` made audit logs self-contained for the matched snippet. The next review gap was the replacement side: supervisors could see what was matched, but not the exact intended replacement without inspecting the JSON payload. `matches[]` now also includes a bounded `new_text_preview`, giving each scoped edit match a compact before/after record.

The `.47` supervised round confirmed `new_text_preview` was useful enough that the agent explicitly cited it while validating a perf HUD stall assertion edit. The remaining friction was preview truncation for longer assertion blocks: the before/after audit existed, but could cut off before the newly inserted fields. `edit.replace_in_member` now accepts `preview_chars` (default `96`, bounded up to `2000`) so agents can request a larger compact audit record for long old/new snippets without rereading the member source.

The `.48` supervised round validated caller-controlled preview lengths: the agent used `preview_chars: 320` for a multi-line A11y assertion insertion and `preview_chars: 256` for a correction, while staying entirely inside roscli for C# work. The next audit gap was that long common prefixes could still consume the preview budget before the inserted fields. Truncated `text_preview` and `new_text_preview` now bias their window toward the first changed character, so the compact before/after record favors the semantic delta rather than the shared prefix.

The `.49` supervised round confirmed change-biased previews were easier to audit: the agent reported that a palette assertion insertion showed the new `state_screen_number` and `state_screen_slug` assertions before truncating, with no C# fallback. The next audit gap was structural: agents still had to visually compare preview strings to locate the divergence. `matches[]` now includes `first_changed_offset`, `first_changed_line_delta`, and `first_changed_column_delta` so tools can jump to or summarize the exact old/new divergence point.

The `.50` supervised round validated the structured divergence fields: the agent cited `first_changed_offset=127`, `first_changed_line_delta=1`, and `first_changed_column_delta=21` while auditing a palette ranking assertion edit. The next ergonomics gap was log readability: offsets are machine-locatable, but compact reports still benefit from a delta-only snippet. `matches[]` now includes `old_change_preview` and `new_change_preview`, short bounded previews centered on the first changed region, separate from the broader old/new previews.

The `.51` supervised round confirmed the delta-only previews made reporting easier: the agent cited `old_change_preview` and `new_change_preview` directly while validating a palette scroll mouse assertion edit. The next tooling gap was absolute navigation. `matches[]` now includes `old_change_line`, `old_change_column`, `new_change_line`, and `new_change_column` derived from the first changed region so audit tools can jump directly to the delta location.
